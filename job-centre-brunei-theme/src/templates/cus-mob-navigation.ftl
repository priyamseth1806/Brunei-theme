<nav class="${nav_css_class} ${cd_dropdown}" id="navigation" role="navigation">
	...
	<ul aria-label="<@liferay.language key="site-pages" />" class="nav navbar-nav navbar-site cd-dropdown-content" role="menubar">
			...
			<#list nav_items as navItem>
					...
					<#if showChildren>
						...
						<#assign
							nav_item_css_class = "${nav_item_css_class} dropdown has-children"
					...
					<li class="${nav_item_css_class} " id="layout_${navItem.getLayoutId()}" ${nav_item_attr_selected} role="presentation">
						<a aria-labelledby="layout_${navItem.getLayoutId()}" ${nav_item_attr_has_popup} class="${nav_item_link_css_class}" ${nav_item_href_link} ${nav_item_link_attr} ${navItem.getTarget()} role="menuitem">
							<span><@liferay_theme["layout-icon"] layout=navItem.getLayout() /> ${navItem.getName()}</span>
						</a>
	
						<#if showChildren>
							<ul aria-expanded="false" class="child-menu cd-secondary-dropdown is-hidden" role="menu">
